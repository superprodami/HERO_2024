...
#define Motar_mode 0	//设置控制模式，为0为IMT模式，为1为位置速度模式，为2为速度模式
#define CANID_RAMMER 0x006
#define CANID_YAW    0x109
#define RAMMER_ContrlID 0x106
#define YAW_ContrlID    0x009
...
extern Motor_4310Type Motor_4310_Rammer,Motor_4310_Yaw,Motor_4310_EnYaw;			



uint8_t CANx_SendStdData(CAN_HandleTypeDef* hcan,uint16_t ID,uint8_t *pData,uint16_t Len);
extern void DM_enable(CAN_HandleTypeDef* _hcan,Motor_4310Type motor,uint16_t ID);
extern void Motor_4310_receive(Motor_4310Type* motor, uint8_t* temp, uint16_t CAN_ID);
extern void Motor_4310_angle_receive(Motor_4310Type* motor, uint8_t* temp, uint16_t CAN_ID);
