...
/**
  ...
  * @retval         none
  */
float Yaw_compensation = 0;//零飘补偿
float Yaw_compensation_add = 0.00009;
...
//				imu_cali_slove(bmi088_real_data.gyro,accel_fliter_3,ist8310_real_data.mag,&bmi088_real_data,&ist8310_real_data);
        ...
        IMU_angle[0] = 180.0f*INS_angle[0]/PI; //yaw
        IMU_angle[1] = 180.0f*INS_angle[1]/PI; //pitch
        IMU_angle[2] = 180.0f*INS_angle[2]/PI; //roll
//				//线性消零飘
				Yaw_compensation += Yaw_compensation_add;
				IMU_angle[0] += Yaw_compensation;
				AngleLoop_f(&IMU_angle[0] ,360.f);

				
				Last_ol[0] = Gimbal.IMUData.ol[0];
				Last_ol[1] = Gimbal.IMUData.ol[1];
				Last_ol[2] = Gimbal.IMUData.ol[2];
				Gimbal.IMUData.ol[0] = IMU_angle[0]*0.017453292f;          					 //yaw
				Gimbal.IMUData.ol[1] = INS_angle[1];           //pitch
				Gimbal.IMUData.ol[2] = INS_angle[2];           //roll
				Gimbal.IMUData.accel[0] = accel_fliter_3[0];
				Gimbal.IMUData.accel[1] = accel_fliter_3[1];
				Gimbal.IMUData.accel[2] = accel_fliter_3[2];
				for(uint8_t i = 0; i < 4; i++)
				{
					Gimbal.IMUData.quat[i] = INS_quat[i];             
				}				
				for(uint8_t i = 0; i < 3; i++)
				{
					Gimbal.IMUData.gyro[i] = bmi088_real_data.gyro[i];    //0 roll    1 pitch     2 yaw
					Gimbal.IMUData.ol_accel[i] = (Gimbal.IMUData.ol[i] - Last_ol[i]) * 1000;
				}	
//-----------------------------------------------------------------------------------
        ...
        vision_transmit();
				...
				{
				set_gimbal_angle(&hcan2,IMU_angle[0]);
				set_gimbal_flag(&hcan2);
				test_can =0;
				}
...
static void imu_temp_control(fp32 temp)
{
    ...
    if (first_temperate)
    {
        PID_calc(&imu_temp_pid, temp, bmi088_real_data.temp);
